...
class LLMFactory:
    ...
    def get_model_configs() -> Dict[str, ModelConfig]:
        ...
        # Local LLM models
        configs["qwen2-0.5b"] = ModelConfig(
            name="qwen2-0.5b",
            provider_type="local_llm",
            model_name="Qwen/Qwen2-0.5B-Instruct"
        )
        
        configs["qwen2-1.5b"] = ModelConfig(
            name="qwen2-1.5b",
            provider_type="local_llm",
            model_name="Qwen/Qwen2-1.5B-Instruct"
        )

        configs["qwen3-7b"] = ModelConfig(
            name="qwen3-7b",
            provider_type="local_llm",
    ...
    async def create_llm_provider(model_config: ModelConfig) -> LLMProvider:
        ...
        if model_config.provider_type == "google_genai":
            from google import genai
            client = genai.Client(api_key=model_config.config["api_key"])
            return LLMProvider(
                client=client,
                deployment_name=model_config.config["model_name"],
                provider_type="google_genai"
            )
        elif model_config.provider_type == "local_llm":
            from llm.local_llm import LocalLLMWrapper
            client = LocalLLMWrapper(model_name=model_config.config["model_name"])
            return LLMProvider(
                client=client,
                deployment_name=model_config.config["model_name"],
                provider_type="local_llm"
