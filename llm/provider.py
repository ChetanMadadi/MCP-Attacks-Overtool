...
class LLMProvider:
    ...
    async def get_completion(self, system_prompt: str, user_prompt: str, max_tokens: int, return_usage: bool = False) -> Any:
        ...
        # Handle Google Genai client differently
        # if self.provider_type == "google_genai":
        #     return await self._get_completion_google_genai(system_prompt, user_prompt, max_tokens, return_usage)
        
        # Handle local LLM client
        if self.provider_type == "local_llm":
            return await self._get_completion_local_llm(system_prompt, user_prompt, max_tokens, return_usage)
    ...
    async def _get_completion_local_llm(self, system_prompt: str, user_prompt: str, max_tokens: int, return_usage: bool = False) -> Any:
        """Get a completion from local LLM client.
        ...
        for attempt in range(max_attempts):
            try:
                logger.info(f"Generating completion using {self.deployment_name} (Local LLM) (attempt {attempt + 1}/{max_attempts}, max_tokens: {max_tokens})")
                
                # Combine system and user prompts for local model
                ...
                # For local models, just retry with exponential backoff
                if attempt < max_attempts - 1:
                    wait_time = 2 ** attempt  # 1, 2 seconds
                    logger.info(f"Waiting {wait_time} seconds before retry...")
                    await asyncio.sleep(wait_time)
