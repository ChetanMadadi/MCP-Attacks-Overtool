...
class BenchmarkRunner:
    ...
    async def execute_single_task_with_model(
        ...
        if not hasattr(self, '_judge_provider') or self._judge_provider is None:
            local_client = LocalLLMWrapper(model_name="Qwen/Qwen2-0.5B-Instruct")
            self._judge_provider = LLMProvider(local_client, "Qwen/Qwen2-0.5B-Instruct", "local_llm")
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog='''
Examples:
  %(prog)s                                    # Run with default settings (qwen2-0.5b, all tasks)
  %(prog)s --models qwen2-0.5b                # Test only qwen2-0.5b
  %(prog)s --models all                       # Test all available models
  %(prog)s --list-models                      # List all available models
  %(prog)s --models qwen2-0.5b --tasks-file tasks/mcpbench_tasks_single_runner_format.json
        '''
    )
    ...
    parser.add_argument(
        ...
        metavar='MODEL',
        default=None,
        help='Models to test. Use "all" for all models, or specify model names. Default: qwen2-0.5b'
    )
...
def _determine_selected_models(args, available_models):
    ...
    if args.models is None:
        # Default to qwen2-0.5b if available, otherwise first available model
        if 'qwen2-0.5b' in available_models:
            selected_models = ['qwen2-0.5b']
